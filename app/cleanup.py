...
with db() as conn:
    expired = conn.execute(
        "SELECT ip, policy_id FROM ip_allowlist "
        "WHERE expires_at IS NOT NULL AND expires_at < ? AND policy_id IS NOT NULL",
        (now,),
    ).fetchall()
...
# Group expired IPs by policy_id
by_policy = {}
for row in expired:
    by_policy.setdefault(row["policy_id"], set()).add(row["ip"])
